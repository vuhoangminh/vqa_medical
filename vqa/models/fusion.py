...
class ElementdivFusion(AbstractFusion):

    def __init__(self, opt):
        super(ElementdivFusion, self).__init__(opt)
        # Modules
    ...
    def forward(self, input_v, input_q):
        ...
        if 'dim_q' in self.opt:
            ...
            x_q = input_q
        # hadamard product
        x_mm = torch.div(x_q, x_v)
        return x_mm


class MinhmulFusion(AbstractFusion):

    def __init__(self, opt):
        super(MinhmulFusion, self).__init__(opt)
        # Modules
        if 'dim_v' in self.opt:
            self.linear_v = nn.Linear(self.opt['dim_v'], self.opt['dim_h'])
        else:
...
class MinhsumFusion(AbstractFusion):

    def __init__(self, opt):
        super(MinhsumFusion, self).__init__(opt)
        # Modules
        if 'dim_v' in self.opt:
            self.linear_v = nn.Linear(self.opt['dim_v'], self.opt['dim_h'])
        else:
    ...
    def forward(self, input_v, input_q):
        ...
        if 'dim_q' in self.opt:
            ...
            x_q = input_q
        # hadamard product
        x_mm = torch.add(x_v, 8, x_q)
        return x_mm


class ElementsumFusion(AbstractFusion):

    def __init__(self, opt):
        super(ElementsumFusion, self).__init__(opt)
        # Modules
        if 'dim_v' in self.opt:
            self.linear_v = nn.Linear(self.opt['dim_v'], self.opt['dim_h'])
        else:
    ...
    def forward(self, input_v, input_q):
        ...
        if 'dim_q' in self.opt:
            ...
            x_q = input_q
        # hadamard product
        x_mm = torch.add(x_q, 1, x_v)
        return x_mm


class ConcatFusion(AbstractFusion):

    def __init__(self, opt):
        super(ConcatFusion, self).__init__(opt)
        # Modules
        if 'dim_v' in self.opt:
            self.linear_v = nn.Linear(self.opt['dim_v'], int(self.opt['dim_h']/2))
        else:
            print('Warning fusion.py: no visual embedding before fusion')

        if 'dim_q' in self.opt:
            self.linear_q = nn.Linear(self.opt['dim_q'], int(self.opt['dim_h']/2))
        else:
            print('Warning fusion.py: no question embedding before fusion')
