...
model_names = pytorch_resnet_names + torch7_resnet_names + ['resnet18_idrid', 'resnet152_idrid']
...
def update_ordereddict(state_dict, old_key, new_key):
    new_state_dict = OrderedDict([(new_key, v) if k == old_key else (k, v) for k, v in state_dict.items()])
