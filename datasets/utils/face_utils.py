...
import numpy as np
import cv2
from face_evoLVe_PyTorch.align.matlab_cp2tform import get_similarity_transform_for_cv2
...
def convert_68_to_5_points(facial68points):
    facial5points = np.zeros((5, 2))
    left_eye = extract_eye_center(facial68points, LEFT_EYE_INDICES)
    right_eye = extract_eye_center(facial68points, RIGHT_EYE_INDICES)
    nose = facial68points[33, :]
    left_mouth = facial68points[48, :]
    right_mouth = facial68points[54, :]
    facial5points[0, :] = left_eye
    facial5points[1, :] = right_eye
    facial5points[2, :] = nose
    facial5points[3, :] = left_mouth
    facial5points[4, :] = right_mouth
...
def extract_eye(shape, eye_indices):
    points = map(lambda i: shape[i, :], eye_indices)
    return list(points)
...
def align_image(input_image, landmark_image, scale=1):
    img = dlib.load_rgb_image(input_image)
    left_eye = extract_left_eye_center(landmark_image)
    right_eye = extract_right_eye_center(landmark_image)
    ...
    rotated = cv2.warpAffine(
        img, M, (s_width, s_height), flags=cv2.INTER_CUBIC)
