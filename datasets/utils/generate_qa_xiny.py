...
def process_one_image(name_image, dataset, disease):
    ...
    if os.path.exists(dir_image_haemorrhages):
        ...
        retval, thresh_gray_image_original = \
            cv2.threshold(gray_image_original, thresh=30,
                          maxval=255, type=cv2.THRESH_BINARY)

        image_haemorrhages = cv2.imread(dir_image_haemorrhages, 1)
        gray_image_haemorrhages = cv2.cvtColor(
            image_haemorrhages, cv2.COLOR_BGR2GRAY)
        retval, thresh_gray_image_haemorrhages = \
            cv2.threshold(gray_image_haemorrhages, thresh=30,
                          maxval=255, type=cv2.THRESH_BINARY)
...
def pad_resize(crop_image_original, crop_image_haemorrhages):
    ...
    resized_image_haemorrhages = cv2.resize(
        padded_image_haemorrhages, (256, 256))
...
def crop_image_remove_vertical_border(
    ...
    points = np.argwhere(thresh_gray_image_original == 255)
    ...
    crop_image_original = thresh_gray_image_original[y:y+h, x:x+w]
    # create a cropped region of the gray image
    crop_image_haemorrhages = thresh_gray_image_haemorrhages[y:y+h, x:x+w]
...
def get_dir_image(dir_folder_original_images, dataset, name_image):
    dir_image = '{}{}/{}.jpg'.format(dir_folder_original_images,
                                     dataset, name_image)
    return dir_image
...
def transform_groundtruth(dir_folder_groundtruth_images, dataset, disease, name_image):
    dir_image_disease = get_dir_image_disease(
        dir_folder_groundtruth_images, dataset, disease, name_image)
