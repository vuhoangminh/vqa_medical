...
IMAGE_SET_DIR = PROJECT_DIR + \
    "/data/raw/idrid/preprocessed/imagesets/"
RAW_DIR = PROJECT_DIR + "/data/vqa_idrid/raw/raw/"
QA_PATH = RAW_DIR + "idrid_qa_gt.csv"
processed_qa_per_question_path = RAW_DIR + "idrid_qa_per_question.csv"
...
def fix_file_id_ext(df, ext="jpg"):
    for index, row in df.iterrows():
        file_id = row["file_id"]
        if ext not in file_id:
            file_id = "{}.{}".format(file_id, ext)

        df.iloc[index, df.columns.get_loc('file_id')] = file_id
    return df
...
def process_df_to_build_json(segmentation_dict_tool=None):
    if os.path.exists(processed_qa_per_question_path):
        ...
        df = fix_file_id_ext(df)

        drop_columns = ['Unnamed: 0', 'haemorrhages',
                        'hard exudates', 'microaneurysms', 'soft exudates']
        for drop_col in drop_columns:
            if drop_col in list(df):
                df = df.drop(columns=[drop_col])
        print(">> insert dataset column")
        df = pd_utils.insert_dataset_to_df(df, segmentation_dict_tool, is_split=False)
        print(">> build full df")
