...
def vqa_processed(params):
    ...
    path_train = os.path.join(
        params['dir'], 'interim', params['trainsplit']+'_questions_annotations.json')
    if params['trainsplit'] == 'train':
        path_val = os.path.join(
            params['dir'], 'interim', 'val_questions_annotations.json')
    path_test = os.path.join(params['dir'], 'interim', 'test_questions.json')
    path_testdev = os.path.join(
        params['dir'], 'interim', 'testdev_questions.json')
    ...
    trainset = process_utils.encode_question(
        trainset, word_to_wid, params['maxlength'], params['pad'])
    if params['trainsplit'] == 'train':
        valset = process_utils.encode_question(
            valset, word_to_wid, params['maxlength'], params['pad'])
    testset = process_utils.encode_question(
        testset, word_to_wid, params['maxlength'], params['pad'])
    testdevset = process_utils.encode_question(
